...
function buildRow({ stationId, price, hoursAgo, lat, lon, fuelType = 'regular' }) {
    return {
        stationId,
        fuelType,
        price,
        timestampMs: Date.UTC(2026, 2, 7, 12, 0, 0) - (hoursAgo * HOUR_MS),
        lat,
        lon,
    };
}
...
test('rejects a stale low replay and replaces it with the predicted price', () => {
    ...
    assert.ok((result.predictedPrice || 0) >= 3.00);
    assert.ok((result.finalDisplayedPrice || 0) >= 3.00);
