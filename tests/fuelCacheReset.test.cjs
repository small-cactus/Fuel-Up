...
process.env.EXPO_PUBLIC_PRIMARY_STATION_URL = process.env.EXPO_PUBLIC_PRIMARY_STATION_URL || 'https://station-provider.invalid/query';
process.env.EXPO_PUBLIC_PRIMARY_STATION_ORIGIN = process.env.EXPO_PUBLIC_PRIMARY_STATION_ORIGIN || 'https://station-provider.invalid';
process.env.EXPO_PUBLIC_PRIMARY_STATION_REFERER = process.env.EXPO_PUBLIC_PRIMARY_STATION_REFERER || 'https://station-provider.invalid/app';
...
function buildStationHistoryRow({
    ...
    return {
        ...
        source_label: 'Dedicated Station Feed',
        rating: 4.2,
...
test('clearFuelPriceCache prevents in-flight requests from repopulating cached fuel data', async () => {
    ...
    console.error = (...args) => {
        if (String(args[0] || '').includes('Dedicated station feed fallback lookup failed')) {
            return;
        }
    ...
    try {
        ...
        const query = {
            ...
            radiusMiles: 10,
            fuelType: 'regular',
            allowLivePrimary: true,
            preferredProvider: 'primary',
            forceLivePrimary: true,
        };
...
test('refreshFuelPriceSnapshot does not call the dedicated station feed without an explicit live opt-in', async () => {
    const asyncStorageMock = createAsyncStorageMock();
    const asyncStoragePath = require.resolve('@react-native-async-storage/async-storage');
    const devCounterPath = require.resolve('../src/lib/devCounter.js');
    const supabasePath = require.resolve('../src/lib/supabase.js');
    ...
    global.fetch = async () => {
        fetchCalled = true;
        throw new Error('Dedicated station feed fetch should not run');
    };
    ...
    try {
        ...
        await assert.rejects(
            ...
            error => {
                assert.equal(
                    error?.userMessage,
                    'No cached nearby dedicated station prices are available yet. Try again after the next hourly refresh.'
                );
                return true;
            }
        );
