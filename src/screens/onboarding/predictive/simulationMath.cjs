...
function getCameraForProgress(routeMetrics, sceneConfig, progress) {
  ...
  const arrivalHeadingBlend = clamp(
    smoothstep(
      Math.max(
        0,
        routeMetrics.totalDistanceMeters - (sceneConfig?.orbit?.headingAlignmentStartMeters || 180)
      ),
      Math.max(
        0,
        routeMetrics.totalDistanceMeters - (sceneConfig?.orbit?.headingAlignmentEndMeters || 16)
      ),
      currentDistanceMeters
    ),
    0,
  ...
  if (!turnContributions.length) {
    return {
      ...
      heading: interpolateHeadingDegrees(baseHeading, routeMetrics.finalHeading, arrivalHeadingBlend),
      pitch: profileValues.pitch,
      altitude,
    };
  }
  ...
  return {
    ...
    heading: interpolateHeadingDegrees(
      interpolateHeadingDegrees(baseHeading, lockedAnticipatedHeading, headingAnticipationBlend),
      routeMetrics.finalHeading,
      arrivalHeadingBlend
    ),
