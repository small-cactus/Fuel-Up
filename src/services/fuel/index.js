...
function mapStationPriceRowToQuote({ row, origin, fallbackSourceLabel }) {
    ...
    const quote = {
        providerId: row.provider_id || 'primary',
        ...
        isEstimated: false,
        sourceLabel: fallbackSourceLabel || row.source_label || PROVIDER_LABELS.primary,
...
async function fetchSecondaryStationQuote({ latitude, longitude, radiusMiles, fuelType, config }) {
    const isConfigured = Boolean(config.secondaryStationUrl && config.secondaryStationApiKey);
    const debugEntry = createDebugEntry('secondary', 'station', isConfigured);

    if (!isConfigured) {
        debugEntry.error = 'Extended station feed is not configured.';
        debugEntry.failureCategory = 'config';
        return { debugEntry, quote: null };
    }

    const requestConfig = buildSecondaryStationRequest({
        latitude,
        longitude,
        radiusMiles,
        config,
    ...
    try {
        const response = await fetchJson(requestConfig.url, {
            ...
            headers: {
                ...
                [config.secondaryStationApiKeyHeader]: config.secondaryStationApiKey,
                ...(requestConfig.fieldMask
                    ? { [config.secondaryStationFieldMaskHeader]: requestConfig.fieldMask }
                    : {}),
            },
            method: 'POST',
        ...
        debugEntry.requests.push(
            createDebugRequest({
                ...
                status: error?.status,
                output: error?.payload || null,
                error: error?.message || 'Extended station feed request failed',
            })
        );
        return { debugEntry, quote: null };
    }
...
async function fetchPrimaryStationQuote({ latitude, longitude, fuelType, config, forceLive = false, allowLive = false }) {
    const isConfigured = Boolean(config.primaryStationUrl);
    const debugEntry = createDebugEntry('primary', 'station', isConfigured);
    ...
    try {
        ...
        if (!error && data && data.length > 0) {
            ...
            const quotes = buildLatestQuotesFromRows({
                ...
                origin,
                fallbackSourceLabel: cachedAreaHistoryRows.length > 0 ? 'Dedicated Station Feed (area cache)' : undefined,
            });
            ...
            if (quotes.length > 0) {
                ...
                if (__DEV__) {
                    console.log(
                        `[FuelUp][Dedicated Station Feed] cache-return: total=${quotes.length} search=${searchLat},${searchLng} fuel=${fuelType}`
                    );
                }
    ...
    if (!allowLive) {
        ...
        debugEntry.requests.push(
            createDebugRequest({
                step: 'live_query',
                url: config.primaryStationUrl || 'station-provider://query',
                status: 200,
                output: 'Skipped: live dedicated station fetch is disabled unless explicitly allowed by the hourly or debug path.',
            })
        );
        return { debugEntry, quotes: [] };
    }
    ...
    if (!isConfigured) {
        debugEntry.failureCategory = 'config';
        debugEntry.error = 'Dedicated station feed is not configured.';
        return { debugEntry, quotes: [] };
    }

    const requestConfig = buildPrimaryStationRequest({
        latitude,
        longitude,
        fuelType,
        config,
    ...
    try {
        ...
        const { incrementApiStat } = require('../../lib/devCounter');
        incrementApiStat('primary');

        const liveQuotes = sanitizeStationQuotesForFuelType(normalizePrimaryStationResponse({
            origin: {
                latitude,
                longitude,
            },
        ...
        try {
            ...
            if (usableAreaRows.length > 0) {
                ...
                const dedupedAreaQuotes = buildLatestQuotesFromRows({
                    rows: usableAreaRows,
                    origin,
                    fallbackSourceLabel: 'Dedicated Station Feed (area cache)',
                });
            ...
            console.error('Dedicated station feed fallback lookup failed:', fallbackLookupError);
        }
        ...
        if (__DEV__) {
            console.log(
                `[FuelUp][Dedicated Station Feed] live-return: apiStations=${stationCount} live=${validatedLiveQuotes.length} fallback=${fallbackQuotes.length} adjusted=${adjustedQuoteCount} total=${quotes.length} missingPriceStations=${debugEntry.summary.missingPriceStationCount || 0} apiOmittedFallback=${debugEntry.summary.apiOmittedStationFallbackCount || 0}`
            );
        }
        ...
        debugEntry.requests.push(
            createDebugRequest({
                ...
                status: error?.status,
                output: error?.payload || null,
                error: error?.message || 'Dedicated station feed request failed',
            })
        );
...
async function getCachedFuelPriceSnapshot({ latitude, longitude, radiusMiles, fuelType, preferredProvider = 'primary' }) {
...
async function refreshFuelPriceSnapshot({
    ...
    fuelType,
    preferredProvider,
    forceLivePrimary = false,
    allowLivePrimary = false,
}) {
    const config = getFuelServiceConfig();
    const useDedicatedProvider = preferredProvider === 'primary' && Boolean(config.primaryStationUrl);
    ...
    const cacheKey = buildCacheKey({
        ...
        fuelType: normalizedFuelType,
        preferredProvider: useDedicatedProvider ? preferredProvider : 'all',
    });
    ...
    request = (async () => {
        const debugState = {
            input: {
                ...
                preferredProvider: useDedicatedProvider ? 'primary' : 'all',
            },
            providers: [],
            requestedAt: new Date().toISOString(),
        };

        const stationFetches = useDedicatedProvider
            ? [
                fetchPrimaryStationQuote({
                    latitude,
                    longitude,
                    fuelType: normalizedFuelType,
                    config,
                    allowLive: allowLivePrimary,
                    forceLive: allowLivePrimary && forceLivePrimary,
                }),
            ]
            : [
                fetchTomTomQuote({
        ...
        const areaFetches = useDedicatedProvider
            ? []
            : [
                fetchCardogQuote({
                    latitude,
