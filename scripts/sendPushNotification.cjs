...
const appConfig = require('./loadAppConfig.cjs');
...
Optional env vars:
  ...
  SUPABASE_URL         Defaults to app config extra.supabase.url
  SUPABASE_KEY         Defaults to app config extra.supabase.key
`);
}
...
async function fetchPushTokens() {
    ...
    if (!url || !key) {
        throw new Error('Supabase credentials are missing. Set SUPABASE_URL/SUPABASE_KEY or use the app config extra.supabase values.');
    }
...
async function sendPushNotification({ token, title, body, apnsConfig }) {
    ...
    if (!apnsConfig.topic) {
        throw new Error('APNS_TOPIC is required or the app config must include expo.ios.bundleIdentifier.');
    }
