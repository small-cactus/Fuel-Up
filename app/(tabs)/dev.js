...
export default function DevStatsScreen() {
    ...
    const [stats, setStats] = useState({ primary: 0, secondary: 0, supabase: 0, barchart: 0, tomtom: 0 });
    ...
    const handleRunHourlyRefreshPath = async () => {
        ...
        try {
            ...
            const primaryDebug = (result?.debugState?.providers || []).find(
                provider => provider?.providerId === 'primary'
            );
            const persistedCount = primaryDebug?.summary?.persistedLiveRowCount;
            const persistError = primaryDebug?.summary?.persistError;
            ...
            Alert.alert(
                ...
                quote
                    ? `${quote.stationName}: $${Number(quote.price).toFixed(2)} (${coords.source}, dedicated live feed)${persistLine}${persistErrorLine}`
        ...
        <ScrollView style={[styles.container, { backgroundColor: bgColor }]} contentContainerStyle={{ paddingBottom: 120 }}>
            ...
            <View style={[styles.card, { backgroundColor: cardColor }]}>
                <View style={styles.row}>
                    <Text style={[styles.label, { color: textColor }]}>Dedicated Feed Cache (Supabase):</Text>
                    <Text style={[styles.value, { color: textColor }]}>{stats.supabase || 0}</Text>
                </View>
                <View style={styles.row}>
                    <Text style={[styles.label, { color: textColor }]}>Dedicated Feed Request:</Text>
                    <Text style={[styles.value, { color: textColor }]}>{stats.primary || 0}</Text>
                </View>
                <View style={styles.row}>
                    <Text style={[styles.label, { color: textColor }]}>Extended Feed Request:</Text>
                    <Text style={[styles.value, { color: textColor }]}>{stats.secondary || 0}</Text>
                </View>
                <View style={styles.row}>
